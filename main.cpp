...
int main(int argc, char *argv[]) {
    ...
    while(!done) {
        ...
        rect.w = VIDEOWIDTH;
        rect.h = VIDEOHEIGHT;
        rect.x = (int)((1. + .5 * sin(0.03 * context.n)) * (WIDTH - VIDEOWIDTH) / 2);
        rect.y = (int)((1. + .5 * cos(0.03 * context.n)) * (HEIGHT - VIDEOHEIGHT) / 2);


        SDL_SetRenderDrawColor(context.renderer, 0, 80, 0, 255);
